...
const { baseStores } = require('./20240406114830-Store');
...
module.exports = {
  ...
  async up(queryInterface) {
    ...
    const transactions = isDevelopment ? baseTransactions.concat(devTransactions) : baseTransactions;
  ...
  async down(queryInterface) {
    ...
    const transactionIds = (isDevelopment ? baseTransactions.concat(devTransactions) : baseTransactions).map(
      (transaction) => transaction.id
    );

    if (transactionIds.length > 0) {
      await queryInterface.bulkDelete('Transactions', { id: transactionIds }, {});
    }
  },
};
