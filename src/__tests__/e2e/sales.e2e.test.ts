...
const createMixture = async (agent: any, productId: string, itemNumber = 1) => {
  ...
  const createResponse = await createRequest;
  expect(createResponse.status).toBe(201);
  return { id: createResponse.body.data.id, name: createResponse.body.data.name };
};
...
describe('Sales API (e2e)', () => {
  ...
  it('creates sales with standard, special, mixture, and mixed items', async () => {
    ...
    expect(standardSaleResponse.body.data.paymentMethod).toBe('CASH');
    ...
    expect(specialSaleResponse.body.data.paymentMethod).toBe('M-PESA');
